...
import "./admin_manage_user.css"

class Admin_manage_users extends Component {
    constructor(props) {
        super(props);
        this.state = {
            "arrOfData": [
    ...
    GetUsers = () => {
        ...
        const url = "http://localhost:4000/register/userdetails"
        axios.get(url).then((response) => {
            ...
            if (response.data.success == true) {
                ...
                console.log("==user data==",this.state.arrOfData)
            }
    ...
    render() {
        return (
            <div className="mb-5 pb-5">
                <AdminNav />
                <h3 id="h1">Manage User</h3>
               {this.state.arrOfData.map((item,i)=>{
                 return <div className="adminmanaguserdiv">
                 <h4 key={i}>{item?.registerData[0]?.name}</h4>
                        <h5 key={i}>{item?.registerData[0]?.phone}</h5>
                        <p key={i}>{item?.registerData[0]?.address}</p>
