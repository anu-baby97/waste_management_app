...
const login_data=require("../model/Logindata")
const check_auth=require("../middleware/check-auth")
const checkAuth = require('../middleware/check-auth')
const login = require('../model/Logindata')

WasteRouter.post('/add', check_auth, (req,res)=>{
    var item={
...
WasteRouter.get('/user-payment',checkAuth,(req,res)=>{
    login.aggregate([{
        $lookup:{
            from:'waste_data', localField:'_id', foreignField:'login_id', as:'WasteData'
        }
    },
    {
        $lookup:{
            from:'payment_data', localField:'_id', foreignField:'login_id', as:'PaymentData'
        } 
    }
]).then((data)=>{
    res.status(200).json({
